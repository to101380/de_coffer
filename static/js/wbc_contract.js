		...
		async function webcoin() {
			...
			var WBC_address ="0xB300640c4241fE8343b1CaC9fB25D9c0d1358897";
			...
			var outstanding_balance = payable_WBC-coffer_WBC_balance;

			if(outstanding_balance/(1*10**18)+0.1<0){
				$("#outstanding_balance").text("0");
				$("#payable_status").text("已解鎖，持續生息，可隨時提領。");
				$("#btn_lock").css("display","none");
				$("#btn_unlock").css("display","block");				
			}else{
				$("#outstanding_balance").text(toPoint_F(outstanding_balance/(1*10**18)+0.1));
